...
import { getEntityManagerForConnection, NAMESPACE_NAME, setEntityManagerForConnection } from './common';
...
export function wrapInTransaction<Func extends (this: any, ...args: any[]) => ReturnType<Func>>(
  ...
  function wrapped(this: unknown, ...newArgs: unknown[]): ReturnType<Func> {
    ...
    const operationId = randomUUID(); //String(new Date().getTime())
    ...
    return context.runAndReturn(async () => {
      ...
      switch (propagation) {
        ...
        case Propagation.NEVER:
          if (currentTransaction) {
            throw new TransactionalError("Found an existing transaction, transaction marked with propagation 'NEVER'");
          }
