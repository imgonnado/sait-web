...
export const getEntityManagerForConnection = (connectionName: string, context: Namespace): EntityManager => {
...
export const getHookInContext = (context: Namespace | undefined): EventEmitter => {
  return context?.get(TYPE_ORM_HOOK_KEY);
};

export const setHookInContext = (context: Namespace, emitter: EventEmitter) => {
